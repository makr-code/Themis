...
protected:
    void SetUp() override {
        ...
        ts_store_ = std::make_unique<themis::TimeSeriesStore>(db_->getRawDB());
    ...
    themis::TimeSeriesStore::DataPoint createDataPoint(
        ...
        themis::TimeSeriesStore::DataPoint point;
        point.metric = metric;
        point.entity = entity;
    ...
    std::unique_ptr<themis::TimeSeriesStore> ts_store_;
...
TEST_F(TimeSeriesStoreTest, PutDataPoint_SinglePoint_Success) {
...
TEST_F(TimeSeriesStoreTest, PutDataPoint_EmptyMetric_ReturnsError) {
...
TEST_F(TimeSeriesStoreTest, PutDataPoint_EmptyEntity_ReturnsError) {
...
TEST_F(TimeSeriesStoreTest, PutDataPoints_BatchWrite_Success) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
...
TEST_F(TimeSeriesStoreTest, Query_SinglePoint_ReturnsCorrectData) {
    auto point = createDataPoint("cpu_usage", "server01", t0_, 75.5);
    ASSERT_TRUE(ts_store_->putDataPoint(point).ok);
    
    themis::TimeSeriesStore::QueryOptions opts;
...
TEST_F(TimeSeriesStoreTest, Query_TimeRange_ReturnsFilteredPoints) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
...
TEST_F(TimeSeriesStoreTest, Query_MultipleEntities_ReturnsAllWhenNoEntityFilter) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
        createDataPoint("cpu_usage", "server01", t0_, 70.0),
...
TEST_F(TimeSeriesStoreTest, Query_WithLimit_ReturnsLimitedResults) {
    std::vector<themis::TimeSeriesStore::DataPoint> points;
    for (int i = 0; i < 100; i++) {
...
TEST_F(TimeSeriesStoreTest, Query_WithTagFilter_ReturnsOnlyMatchingPoints) {
...
TEST_F(TimeSeriesStoreTest, Query_EmptyMetric_ReturnsError) {
    themis::TimeSeriesStore::QueryOptions opts;
...
TEST_F(TimeSeriesStoreTest, Aggregate_ComputesCorrectStatistics) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
        createDataPoint("cpu_usage", "server01", t0_, 70.0),
...
TEST_F(TimeSeriesStoreTest, Aggregate_EmptyResult_ReturnsZeroStats) {
    themis::TimeSeriesStore::QueryOptions opts;
...
TEST_F(TimeSeriesStoreTest, Aggregate_SinglePoint_ReturnsCorrectStats) {
    auto point = createDataPoint("cpu_usage", "server01", t0_, 75.5);
    ASSERT_TRUE(ts_store_->putDataPoint(point).ok);
...
TEST_F(TimeSeriesStoreTest, Performance_Query1000Points_UnderThreshold) {
...
TEST_F(TimeSeriesStoreTest, Performance_BatchWrite1000Points_Fast) {
    std::vector<themis::TimeSeriesStore::DataPoint> points;
    for (int i = 0; i < 1000; i++) {
...
TEST_F(TimeSeriesStoreTest, GetStats_ReturnsAccurateMetrics) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
        createDataPoint("cpu_usage", "server01", t0_, 70.0),
...
TEST_F(TimeSeriesStoreTest, GetStats_EmptyStore_ReturnsZeros) {
    auto stats = ts_store_->getStats();
...
TEST_F(TimeSeriesStoreTest, DeleteOldData_RemovesOldPoints) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
        createDataPoint("cpu_usage", "server01", t0_, 70.0),
        createDataPoint("cpu_usage", "server01", t1_, 75.0),
        createDataPoint("cpu_usage", "server01", t2_, 80.0),
...
TEST_F(TimeSeriesStoreTest, DeleteMetric_RemovesAllPointsForMetric) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
        createDataPoint("cpu_usage", "server01", t0_, 70.0),
    ...
    themis::TimeSeriesStore::QueryOptions opts1;
    ...
    themis::TimeSeriesStore::QueryOptions opts2;
...
TEST_F(TimeSeriesStoreTest, Clear_RemovesAllData) {
    std::vector<themis::TimeSeriesStore::DataPoint> points = {
        createDataPoint("cpu_usage", "server01", t0_, 70.0),
        createDataPoint("memory_usage", "server01", t0_, 80.0)
    };
...
TEST_F(TimeSeriesStoreTest, RealWorld_MonitoringPipeline) {
    ...
    themis::TimeSeriesStore::QueryOptions recent_opts;
    ...
    themis::TimeSeriesStore::QueryOptions agg_opts;
...
TEST_F(TimeSeriesStoreTest, RealWorld_AlertingWithThresholds) {
